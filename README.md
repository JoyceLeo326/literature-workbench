...
1. **选一个研究场景**：课程汇报、论文综述或工作简报只会预填工作骨架，不会塞入虚构文献。也可以从空白项目开始。
2. **建立研究项目**：填写真实研究问题、年份范围、研究阶段、交付目标、目标数量、纳入条件、排除条件、中英文概念组与可投入时间。
3. **比较三条研究策略**：文径会把研究阶段、题录进度、语言缺口和期限放在一起，给出三条不同路线。每条都说清收益、代价、适用理由和第一步。
4. **人工确认 V1**：选择路线并勾选接受对应取舍后，才会形成可交付的策略版本。没有人工确认时，下载不会被当作已批准结果。
5. **建立检索记录**：由中英文概念组生成布尔检索式，同时记录数据库、检索日期、结果数量和说明。生成的检索式仍需按目标数据库语法人工校正。
6. **维护题录与筛选**：可手动录入，或导入 CSV、JSON、BibTeX、RIS；逐条补充作者、单位、来源、年份、摘要、DOI、链接和 PDF 文件名，并保存纳入、排除、待定及排除理由。
7. **提取与综合证据**：记录核心发现、人工证据等级和主题标签，再检查来源、新近性、反证与分歧。
8. **把真实反馈带回来**：如果你发现范围太宽、来源不足、结论过强或难以交接，可以写下原因。文径先摆出变化，保留旧版；只有重新确认后，才形成 V2 或后续版本。
9. **导出交付物**：下载 UTF-8 BOM CSV、完整 JSON、BibTeX、Markdown 证据综合、质量报告与策略版本档案；完整 JSON 可再次导入并继续工作。

### 第一次体验建议

打开在线入口后，先选择一个场景骨架，或直接从空白开始。骨架只设置节奏、目标数量与纳入/排除提示，不会把演示题录混进正式项目：

1. 选择课程研究、论文综述或决策简报，再把标题和问题改成你手上的真实任务；
...
- 一条始终可见的“当前阶段—本步结果—下一动作”提示；手机端收束为“开始、找证据、做判断、带走”四章；
